...
**Video bitrate + congestion control.** Per-sender 800 kbps ceiling (`bitrateCapFor`, shrinks across senders as the roster grows), a freeze-hygiene cap not a freeze fix, shaped at runtime by per-peer AIMD congestion control with a learned soft ceiling and resolution hysteresis (`monitorCongestion`/`applyVideoBitrateCaps`). The full algorithm and its don't-touch invariants are the deep dive in **[video.md](video.md)**.
...
**Screen share (Phase 4, 2.0.0).** A SECOND video source on the single video slot, mutually exclusive with the camera (`setScreenShareEnabled`). Camera↔screen swaps the source on the existing m-line via `replaceTrack` (instant, no reneg); first-enable `addTrack`s + renegotiates. Screen VIDEO parks its sender on stop (`replaceTrack(null)`, reused via `idleVideoSender`, which must only match an already-SEND transceiver — never a recvonly receive slot, the 2.0.0 regression). Screen AUDIO is `addTrack`ed into the mic's `localStream` (rides its own m-line, so muting the mic never silences it) and FULLY removed on teardown (no `video_muted`-style gate). A screen rides its OWN, far higher bitrate budget (`SCREEN_MAX_BITRATE_BPS` 2.5M / `SCREEN_TOTAL_UPLINK_BPS` 4.5M, camera path untouched), a res ladder rescaled to it, VP9-first codec, and a bandwidth soft ceiling decoupled from CPU limitation — its full congestion behaviour (incl. steps resolution DOWN rather than shedding framerate, `detectScreenMotion`'s `contentHint` motion↔detail flip) is the deep dive in **[video.md](video.md)**. UI lives in `voiceui.js`; `web/e2e/screen-share` pins share→receive→camera-swap→teardown.
