...
- **The INITIAL offer belongs to `onVoiceState` alone — `sendOffer` (the `negotiationneeded` path) returns early while `!pc.remoteDescription`.** Adding our tracks to a fresh peer fires `negotiationneeded`; letting it offer too collides with the deterministic initial offer at *every* call setup, and that needless glare's rollback churn intermittently stalls ICE before it connects (was the flaky e2e symptom: the call never reaches `connected`, so a later camera tile never goes live). Camera-on-at-join is handled post-connect, not by racing init — see next point.
- **Glare re-offer is a ONE-SHOT, never re-evaluated per settle.** When the polite peer rolls back its own offer to accept the impolite peer's, the track it had added (e.g. its camera) is left un-negotiated. After answering, `onOffer` sets `renegotiatePending` iff `hasUnsentLocalTrack(pc)` (a local track whose `currentDirection` isn't sending), and `maybeRenegotiate` re-offers it once stable. Gating on `hasUnsentLocalTrack` avoids a redundant re-offer for sends the accepted offer already covered (audio at join) that would stall connecting ICE. **Do NOT** wire this to `signalingstatechange`/re-check live transceiver state each settle — both peers then re-offer in lockstep and oscillate, breaking *both* video directions (measured 13/25 glare failures vs 0/25 with the one-shot flag).
...
- **Quick hang-up-then-call must not collide with the previous call's teardown.** `leaveVoiceChannel`/`endCallLocally` close peer connections *synchronously* (`finishTeardown` → `closeAllPeers` before the farewell-tone `await`), so a new call can't reuse a half-closed peer (which produced an m-line-order mismatch that killed the new call). The mic/meter release stays deferred behind the tone but is guarded by `callGen`: a call started during the tone wait bumps it, and the stale teardown then stops only the old stream it captured, never the new call's. `TestRapidReJoin*` (voice.test.js) guards it.
- **Pure helpers are all unit-tested in voice.test.js:** `computeRMS`, `clampVolume`, `pttShouldFire`, `pttKeyLabel`, `micErrorMessage`, `reconnectPlan`, `restartOutcome`, `politeFor`, `effectiveConnectionState`, `withVideoBitrateCap`, `orderVideoCodecsVP8First`, `hasUnsentLocalTrack`.
