...
- Screen AUDIO (Chrome tab/system audio) rides its OWN dedicated `MediaStream` (own msid via `attachScreenAudioToPeers`), so the remote splits it onto a SECOND per-peer `<audio>` element (`screenAudioEls`, distinguished by msid — `voiceStreamIds` records the FIRST audio stream seen for a peer as their voice; any other stream is the share) and gives it an INDEPENDENT playout volume (`get/setScreenVolumeForUser`, persisted under `rivendell.voiceScreenVolumes`; surfaced as the per-sharer "stream volume" slider in the members panel + the DM header, shown only while `hasScreenAudio`). It is deliberately NOT a member of `localStream`, so `setVoiceMuted`'s `getAudioTracks()` loop never touches it — muting the mic never silences the share. It rides its OWN m-line. Teardown FULLY removes it (`pc.removeTrack`, NOT park) — audio has no `video_muted`-style gate, so a parked-but-lingering track would still be heard; the receiver also tears down the screen `<audio>` element on that track's `onended` (and re-renders so the slider disappears). `web/e2e/screen-share` pins share→receive(2 elements + the separate slider)→camera-swap→teardown.
