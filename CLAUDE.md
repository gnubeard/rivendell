...
**Link preview proxy.** `GET /api/link-preview?url=<https-url>` fetches OG/Twitter card meta from an explicitly allowlisted set of hosts and returns `{title, description, image}`. Two allowlists, both in `handlers.go`, gated through `previewHostAllowed`: exact-match hosts (`allowedPreviewHosts`: `bsky.app`, `twitter.com`, `x.com`, `xcancel.com`, `github.com`, `www.github.com`) and registrable-domain suffixes that also match any subdomain (`allowedPreviewSuffixes`: `wikipedia.org` for per-language subdomains). Suffix matching is `host == suffix || host endswith "."+suffix`, so a lookalike like `notwikipedia.org` is rejected. (Tumblr was previously allowlisted but was removed: it hard-blocks our server's IP at the edge — every request, any User-Agent or endpoint, returns nginx 403 — so no preview was reachable to fetch.) The client mirrors this in `PREVIEW_HOST_RE` (`format.js`) — keep the two in sync. Any fetch error or non-2xx returns an empty object (never 5xx). Non-https URLs and non-allowlisted hosts return 400. Never proxy arbitrary URLs.
