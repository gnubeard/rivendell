...
import { formatMessage, parsePermalink, extractHideURL, extractFirstBareURL, isImageURL, suppressEmbedURL, replySnippet, dataUriToFile, shouldGroupMessage, BUILTIN_EMOJI } from "./format.js";
...
import { humanBytes, overSizeLimit, initials } from "./util.js";
...
import { liveDeletedStillLoaded } from "./grouping.js";
...
//     │         ├─ reactionsRow(m)                      // pills (`mine` rule)
//     │         └─ messageActions(m, {…})               // hover edit/forward/pin/del/read
//     │       (messageRow + the builders above live in messagelist.js; this loop
//     │        drives them via the createMessageList returns)
...
// The message-row construction cluster lives in messagelist.js. It's READ-ONLY on
// state + the view flags; app.js keeps the orchestration (renderMessages / append /
// patch / optimistic-send) that calls these returns. editorFor/avatarSrc/jumpToMessage
// are hoisted function declarations and buildLinkPreview just resolved above, so the
// factory can build here. The returns are destructured into same-named consts so the
// call sites across the orchestration paths read textually unchanged.
const messageList = createMessageList({
  el,
  getState: () => state,
  getViewFlags: () => ({ editingMessageId, flashMessageId }),
  editorFor,
  buildLinkPreview,
  avatarSrc,
  emojiURL: (code) => api.emojiURL(code),
...
const { messageRow, reactionsRow, rowContextFor, groupingAnchor, insertionPointFor, renderDeletedRun, renderSystemMessage } = messageList;
...
// Pinned-messages panel — pins.js, e2e/pins.spec. reactionsRow (from messagelist.js)
// is injected so the modal can render pills for pins it fetched itself; togglePin/
// toggleReaction (the actions) stay in app.js (the `mine` invariant).
